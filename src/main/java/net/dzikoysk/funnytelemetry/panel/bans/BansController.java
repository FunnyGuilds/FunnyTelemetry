...
public class BansController
{
    ...
    {
        this.banService.findBan(UUID.fromString(banId))
                .ifPresentOrElse(
                        ban -> model.addAttribute("ban", ban),
    ...
    {
        this.banService.findBan(UUID.fromString(banId))
                .filter(ban -> ! ban.isRevoked())
                .ifPresent(ban -> {
                    this.banService.revokeBan(ban, revokeForm.getReason(), principal.getName());
                    this.logService.submitLog(ActionType.REVOKED_BAN, ban.getUniqueId().toString(), principal.getName(), request.getRemoteAddr());
                });
